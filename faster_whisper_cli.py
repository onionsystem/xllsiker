...
try:
    ...
    if filler_pass:
        # ─── FILLER PASS: 2 kez çalıştır, her ikisinde de çıkanları al ───
        # no_speech_threshold=0.3 → belirsiz sesleri "konuşma yok" deyip atlamıyor
        # 2. geçişte temperature=0.2 → farklı yol, aynı filler'ı bulursa güvenilir
        log(f"Filler pass prompt: {filler_prompt}")

        def _run_fp(temperature_val, pass_num):
            s, i2 = model.transcribe(
                audio_path,
                language=lang_arg,
            ...
            for seg in s:
                ...
                for w in seg.words:
                    ...
                    if dur <= 1.0 and (prob <= 0.90 or len(wt) <= 3):
                        log(f"  FP{pass_num}-WORD: '{raw_word.strip()}' norm:'{wt}' {w.start:.2f}-{w.end:.2f}s prob:{prob:.2f} dur:{dur:.2f}")
        ...
        log("Filler pass 1/2 (temperature=0.0)...")
        cuts1, detected = _run_fp(0.0, 1)
        log(f"  Pass 1: {len(cuts1)} filler")

        log("Filler pass 2/2 (temperature=0.2)...")
        ...
        def _merge_fp_cuts(a, b, tol=0.15):
            merged = list(a)
            for cb in b:
                # a listesinde yakın bir eleman var mı?
                already = any(
                    abs(cb['start'] - ca['start']) <= tol and
                    abs(cb['end'] - ca['end']) <= tol
                    for ca in a
                )
                if not already:
                    merged.append(cb)
        ...
        filler_cuts = _merge_fp_cuts(cuts1, cuts2)
        log(f"Filler pass sonuc: {len(filler_cuts)} filler bulundu (pass1:{len(cuts1)} pass2:{len(cuts2)} union:{len(filler_cuts)})")
        for fc in filler_cuts:
            log(f"  FP-FILLER: '{fc['word']}' {fc['start']:.2f}-{fc['end']:.2f}s reason:{fc['reason']}")
    ...
    if dev == 'cuda':
        try:
            ...
            segs, info = pipe.transcribe(
                ...
                language=lang_arg,
                word_timestamps=True,
                batch_size=min(p['batch_size'], 8),  # kısa klipli ses için daha küçük batch
                vad_filter=True,
                vad_parameters=vad,
                initial_prompt=filler_prompt or None
            )
